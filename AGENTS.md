...
```
/ (حزين)
├── index.html                ← PWA main entry (React)
├── maze.html                 ← Maze game (standalone, no build)
├── manifest.json             ← PWA manifest
├── sw.js                     ← Service worker (cache strategy)
├── AGENTS.md                 ← This file (guidelines + structure)
├── .gitignore
│
├── assets/
│   ├── index-new.js          ← React bundle (minified, 167 lines)
│   └── index-DtXZcElU.css    ← Tailwind CSS bundle (minified)
│
├── images/
│   ├── icon-heart.svg        ← PWA icon / favicon
│   ├── ending-bg.svg         ← Ending/overlay background
│   └── photo-1.jpeg .. 6     ← Gallery photos
│
└── media/
    ├── 04_-_Maak_Alby.mp3    ← Background music (referenced in bundle)
    ├── video.mp4              ← Love story video (referenced in bundle)
    ├── bg-audio.mp3           ← Unused / future use
    ├── bg-audio.mp4           ← Unused / future use
    └── WhatsApp Video ....mp4 ← Extra video (unreferenced)
```

### Key Path References (for editing)

| File | Referenced In | Type |
|---|---|---|
| `./04_-_Maak_Alby.mp3` | `index-new.js:167`, `index.html:28`, `sw.js:2` | Audio |
| `./media/video.mp4` | `index-new.js:167` | Video |
| `./images/photo-*.jpeg` | `index-new.js:167` (config) | Gallery |
| `./maze.html` | `sw.js:2` | Game page |

### How to Edit the React App

The main JS is a minified bundle (`assets/index-new.js`). To edit:
1. Modify `index-new.js` directly for minor text/config changes (search for the Arabic text)
2. For major changes, rebuild with Vite + React + Tailwind

### Login Password
`Jj12` (hardcoded in bundle)

### Couple Names
- هو: **محمد** (was جميل)
- هي: **مي** (was جودي)
- النصوص موجهة من محمد لمي (ولد → بنت)
